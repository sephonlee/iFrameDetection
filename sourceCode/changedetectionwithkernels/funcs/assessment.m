...
%
%   2012 - M. Volpi added user's/producer's accuracies
...
switch lower(par)
    case {'class'}
        ...
        RESULTS.Z               = Z;
        RESULTS.CI              = CI;
        RESULTS.User           = diag(ConfusionMatrix)./sum(ConfusionMatrix,2).*100;
        RESULTS.Prod            = (diag(ConfusionMatrix)'./sum(ConfusionMatrix,1))'.*100;
